...
function BlurrableImage({
  ...
  const jsImgEl = React.cloneElement(img, {
    ...
    className: classNames(img.props.className, 'transition-opacity', {
      'opacity-0': !visible,
    }),
  });
    ...
    <div {...rest}>
      {visible ? (
        <>
          ...
          <img key={url} src={url} className={img.props.className} alt={img.props.alt} />
          <div className={classNames(img.props.className, 'backdrop-blur-xl')} />
        </>
      ) : null}
      {jsImgEl}
